...
maxInterval
```
The 5-minute interval that has the **highest** number of steps, on average, is `r maxInterval`, that is, `r as.numeric(maxInterval) %/% 100` hours and `r as.numeric(maxInterval) %% 100` minutes. On average `r round(avgSteps[maxInterval])` steps are taken in that interval.
...
n.missing
```
There are `r n.missing` **missing values** in the dataset.
